...
const firebaseConfig = {
  ...
  authDomain: "h// --- CONFIGURACIÓN DE FIREBASE ---
const firebaseConfig = {
  apiKey: "AIzaSyCLrVUpGCTxFxuMR0ATlwj2t3osSP0dD7Y",
...
// Función para copiar el código automáticamente al portapapeles
function copiarCodigoAlPortapapeles(codigo) {
  navigator.clipboard.writeText(codigo).then(() => {
    alert(`¡Código "${codigo}" copiado al portapapeles! Ya puedes pegarlo donde gustes.`);
  }).catch(err => {
...
function renderProyectosAdmin() {
  ...
  // --- CÁLCULO DE REPORTE MENSUAL (ROBUSTO) ---
  const selectMes = document.getElementById('filtro-mes');
  ...
  let proyectosFiltradosMes = proyectos.filter(p => {
    if (!p.fechaEntrega || p.fechaEntrega.trim() === '') {
      return true; // Incluir si no tiene fecha para evitar ceros
    }
    if (mesSeleccionado) {
        ...
        <div id="info-view-${index}" style="flex: 1; min-width: 280px;">
          <div style="display: flex; align-items: center; gap: 0.5rem; flex-wrap: wrap;">
            <div style="display: flex; align-items: center; gap: 0.4rem;">
              ...
              <button type="button" onclick="copiarCodigoAlPortapapeles('${p.codigo}')" title="Copiar código automáticamente" style="background: rgba(255,255,255,0.1); color: #fff; border: 1px solid rgba(255,255,255,0.2); padding: 0.2rem 0.5rem; border-radius: 4px; cursor: pointer; font-size: 0.75rem;">
...
function notificarWhatsApp(index) {
  ...
  // Código envuelto en asteriscos (*${p.codigo}*) para que llegue en negrita destacada a WhatsApp
...
function exportarReporteExcel() {
  ...
  let csvContent = "\uFEFF"; // BOM para caracteres latinos correctos en Excel
