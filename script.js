...
const firebaseConfig = {
  apiKey: "AIzaSyCLrVUpGCTxFxuMR0ATlwj2t3osSP0D7Y",
...
let proyectos = [], ingresos = [], portafolio = [], esAdmin = false;
...
async function buscarProyectoPublico(codigo) {
  ...
  try {
    const snap = await db.collection("proyectos_publicos")
      .where("codigo", "==", codigo)
      .limit(1)
      .get();
...
function renderProyectosAdmin() {
  ...
  proyectos.forEach((p, index) => {
    ...
    card.style.cssText =
      "background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.1);border-radius:12px;padding:1.2rem;margin-bottom:1rem;";
          ...
          <div style="flex:1;min-width:260px;">
            ...
            <div style="display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:10px;margin-top:12px;">
              ...
              <div class="financial-box"
                style="background:rgba(56,189,248,.07);border:1px solid rgba(56,189,248,.25);color:#38bdf8;">
                <span style="font-size:.75rem;">Monto total</span>
                <strong style="font-size:1.05rem;white-space:nowrap;">
                  Bs. ${formatearMonto(presupuesto)}
                </strong>
              </div>

              <div class="financial-box"
                style="background:rgba(245,158,11,.07);border:1px solid rgba(245,158,11,.25);color:#f59e0b;">
              ...
              <div class="financial-box"
                style="background:rgba(239,68,68,.07);border:1px solid rgba(239,68,68,.25);color:#ef4444;">
...
async function cambiarEstadoPorId(id, etapaIdx, progreso) {
  ...
  try {
    ...
    batch.update(db.collection("proyectos").doc(id), {
      estado,
      progreso,
      detalles
    });
...
async function eliminarProyecto(id) {
  if (!esAdmin || !auth.currentUser ||
      !confirm("¿Seguro que quieres eliminar este proyecto?")) return;
  ...
  try {
    ...
    if (proyecto) {
      const publico = await db.collection("proyectos_publicos")
        .where("codigo", "==", proyecto.codigo)
        .limit(1)
        .get();

      publico.forEach(doc => batch.delete(doc.ref));

      const ingreso = await db.collection("ingresos")
        .where("proyectoId", "==", id)
        .limit(1)
        .get();

      ingreso.forEach(doc => batch.delete(doc.ref));
    }
    ...
    <div style="display:flex;flex-direction:column;gap:7px;">
      ...
      <div>
        <button type="button"
          ...
          style="background:#16a34a;color:#fff;border:none;padding:8px 12px;border-radius:6px;cursor:pointer;">
          Guardar
        </button>

        <button type="button"
          onclick="renderProyectosAdmin()"
          style="background:#404040;color:#fff;border:none;padding:8px 12px;border-radius:6px;cursor:pointer;">
          Cancelar
        </button>
      </div>
...
async function guardarEdicionInline(id, index) {
  ...
  try {
    ...
    if (!ingresoSnap.empty) {
      const doc = ingresoSnap.docs[0];
      const data = doc.data();
      const cobrado = adelanto + (Number(data.pagosFinales) || 0);
    ...
    const publicoSnap = await db.collection("proyectos_publicos")
...
function renderGestionIngresos() {
  ...
  document.getElementById("ing-resumen-proyectos")?.replaceChildren(
    document.createTextNode(lista.length)
  );

  document.getElementById("ing-resumen-cobrado")?.replaceChildren(
    document.createTextNode(`Bs. ${formatearMonto(totalCobrado)}`)
  );

  document.getElementById("ing-resumen-pendiente")?.replaceChildren(
    document.createTextNode(`Bs. ${formatearMonto(totalPendiente)}`)
  );
  ...
  if (!lista.length) {
    container.innerHTML =
      `<div style="text-align:center;color:#777;padding:25px;">No hay registros para este mes.</div>`;
    return;
  }
  ...
  lista.forEach(ingreso => {
    ...
    card.style.cssText =
      "background:rgba(255,255,255,.035);border:1px solid rgba(255,255,255,.09);border-radius:9px;padding:10px;margin-bottom:7px;";
    ...
    card.innerHTML = `
      ...
      <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:5px;margin-top:8px;">
        <div class="summary-box amber">
          Adelanto<strong>Bs. ${formatearMonto(adelanto)}</strong>
        </div>

        <div class="summary-box green">
          Cobrado<strong>Bs. ${formatearMonto(cobrado)}</strong>
        </div>

        <div class="summary-box red">
          Pendiente<strong>Bs. ${formatearMonto(pendiente)}</strong>
        </div>
      </div>
      ...
      ${pendiente > 0 ? `
        <div style="display:flex;gap:5px;margin-top:8px;">
          ...
          <button type="button"
            ...
            style="background:#16a34a;color:#fff;border:none;border-radius:6px;padding:7px 10px;cursor:pointer;font-weight:bold;">
            Registrar pago
          </button>
        </div>
      ` : `
...
async function exportarIngresosPDF() {
  ...
  if (!filtro || !window.jspdf?.jsPDF)
    return alert("jsPDF no disponible o sin filtro.");
  ...
  doc.setTextColor(105, 105, 105);
  doc.text(
    `Generado el ${gen.toLocaleDateString("es-BO")} a las ${gen.toLocaleTimeString("es-BO", {
      hour: "2-digit",
      minute: "2-digit"
    })}`,
    283,
...
async function subirArchivoCloudinary(archivo, progresoCallback) {
  ...
  const tipoVideo = archivo.type.startsWith("video/");
  const recurso = tipoVideo ? "video" : "image";
  ...
  return new Promise((resolve, reject) => {
    ...
    xhr.upload.addEventListener("progress", e => {
      if (e.lengthComputable)
        progresoCallback?.((e.loaded / e.total) * 100);
    });

    xhr.onload = () => {
      if (xhr.status < 200 || xhr.status >= 300)
        return reject(new Error(`Cloudinary respondió ${xhr.status}`));

      try {
        const data = JSON.parse(xhr.responseText);

        resolve({
          tipo: tipoVideo ? "video" : "imagen",
          url: data.secure_url,
...
async function cargarPortafolioPublico() {
  ...
  try {
    const snap = await db.collection("portafolio")
      .orderBy("creadoEn", "desc")
      .get();

    portafolio = snap.docs.map(doc => ({
      id: doc.id,
      ...doc.data()
    }));
    ...
    container.innerHTML = `
      <div class="portfolio-empty">
        ...
        <p>Portafolio disponible próximamente.</p>
      </div>`;
  }
}
...
async function cargarPortafolioAdmin() {
  ...
  try {
    ...
    console.error("Error portafolio admin:", error);
  }
}
    ...
    container.innerHTML = `
      <div class="portfolio-empty">
        <i class="fa-solid fa-images"
          style="font-size:2rem;color:#9c7151;margin-bottom:10px;"></i>
        <p>Próximamente mostraremos trabajos aquí.</p>
      </div>`;
    return;
  }
  ...
  portafolio.forEach((trabajo, idx) => {
    ...
    card.innerHTML = `
      <div class="portfolio-media"
        ...
        ${pMedia.tipo === "video"
          ? `
            <video src="${pMedia.url}" muted playsinline preload="metadata"></video>
            <div class="portfolio-video-badge">
              <i class="fa-solid fa-video"></i> Video
            </div>`
          : `
            <img src="${pMedia.url}" alt="" loading="lazy">
          `
        }
        ...
        ${media.length > 1
          ? `
            <div class="portfolio-video-badge"
              ...
              <i class="fa-solid fa-images"></i> ${media.length}
            </div>`
          : ""
        }
      </div>
      ...
      <div class="portfolio-card-body">
        <div class="portfolio-card-title">
          ${escaparHTML(trabajo.titulo || "Proyecto")}
        </div>

        <div class="portfolio-card-description">
          ${escaparHTML(trabajo.descripcion || "")}
        </div>

        <button type="button"
          onclick="abrirGaleriaPortafolio(${idx})"
          style="margin-top:12px;width:100%;border:none;padding:10px;border-radius:7px;cursor:pointer;background:#9c7151;color:#fff;font-weight:bold;">
          <i class="fa-solid fa-expand"></i> Ver trabajo
        </button>
      </div>`;
...
function crearModalGaleria() {
  ...
  modal.innerHTML = `
    <div id="portfolio-gallery-overlay"
      style="position:fixed;inset:0;background:rgba(0,0,0,.92);z-index:99999;display:flex;align-items:center;justify-content:center;padding:20px;">

      <button type="button"
        onclick="cerrarGaleriaPortafolio()"
        style="position:fixed;top:20px;right:25px;z-index:100001;width:45px;height:45px;border:none;border-radius:50%;background:rgba(255,255,255,.12);color:#fff;font-size:22px;cursor:pointer;">
        ✕
      </button>

      <button type="button"
        onclick="mediaGaleriaAnterior()"
        style="position:fixed;left:20px;top:50%;transform:translateY(-50%);z-index:100001;width:50px;height:50px;border:none;border-radius:50%;background:rgba(255,255,255,.12);color:#fff;font-size:25px;cursor:pointer;">
        ‹
      </button>

      <div id="portfolio-gallery-content"
        style="max-width:95vw;max-height:90vh;display:flex;align-items:center;justify-content:center;position:relative;">
      </div>

      <button type="button"
        onclick="mediaGaleriaSiguiente()"
        style="position:fixed;right:20px;top:50%;transform:translateY(-50%);z-index:100001;width:50px;height:50px;border:none;border-radius:50%;background:rgba(255,255,255,.12);color:#fff;font-size:25px;cursor:pointer;">
        ›
      </button>

      <div id="portfolio-gallery-counter"
        style="position:fixed;bottom:20px;left:50%;transform:translateX(-50%);color:#fff;background:rgba(0,0,0,.6);padding:7px 14px;border-radius:20px;font-size:13px;">
      </div>
  ...
  document.getElementById("portfolio-gallery-overlay")
    .addEventListener("click", e => {
      if (e.target.id === "portfolio-gallery-overlay")
...
function actualizarGaleria() {
  ...
  if (mediaActual < 0)
    mediaActual = trabajo.media.length - 1;

  if (mediaActual >= trabajo.media.length)
    mediaActual = 0;
  ...
  if (counter)
    counter.innerText = `${mediaActual + 1} / ${trabajo.media.length}`;
}
...
function mostrarPreviewArchivos() {
  ...
  const fotos = Array.from(
    document.getElementById("portfolio-fotos")?.files || []
  );

  const videos = Array.from(
    document.getElementById("portfolio-videos")?.files || []
  );
...
async function publicarTrabajoPortafolio(e) {
  ...
  if (!esAdmin || !auth.currentUser)
    return alert("Inicia sesión como administrador.");

  const titulo =
    document.getElementById("portfolio-titulo")?.value.trim();

  const descripcion =
    document.getElementById("portfolio-descripcion")?.value.trim() || "";

  const archivos = [
    ...Array.from(document.getElementById("portfolio-fotos")?.files || []),
    ...Array.from(document.getElementById("portfolio-videos")?.files || [])
  ];

  if (!titulo || !archivos.length)
    return alert("Escribe un título y selecciona al menos una foto o video.");
  ...
  try {
    ...
    for (let i = 0; i < archivos.length; i++) {
      ...
      const res = await subirArchivoCloudinary(archivo, progreso => {
        if (progBar) {
          progBar.style.width =
            `${((i + progreso / 100) / archivos.length) * 100}%`;
        }
      });
    ...
    if (progText)
      progText.innerText = "Guardando información...";
...
function renderPortafolioAdmin() {
  ...
  if (!portafolio.length) {
    container.innerHTML =
      `<div style="text-align:center;color:#777;padding:25px;">Todavía no tienes trabajos publicados.</div>`;
    return;
  }
  ...
  portafolio.forEach((trabajo, index) => {
    ...
    card.innerHTML = `
      ${pMedia?.tipo === "video"
        ? `<video src="${pMedia.url}" class="portfolio-admin-thumb" muted preload="metadata"></video>`
        : `<img src="${pMedia?.url || ""}" class="portfolio-admin-thumb" alt="">`
      }
      ...
      <div class="portfolio-admin-actions">
        <button type="button"
          ...
          style="background:#3b82f6;color:#fff;border:none;padding:7px 10px;border-radius:6px;cursor:pointer;">
...
async function eliminarTrabajoPortafolio(id) {
  if (!esAdmin || !auth.currentUser ||
      !confirm("¿Seguro que quieres eliminar este trabajo?")) return;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  formLogin?.addEventListener("submit", async e => {
    ...
    if (!email || !password) {
      if (errorMsg) {
        errorMsg.textContent = "Ingresa tu correo y contraseña.";
        errorMsg.classList.remove("hidden");
      }
      return;
    }
  ...
  formBuscar?.addEventListener("submit", async e => {
    ...
    const codigo =
      document.getElementById("input-codigo")?.value.trim().toUpperCase();
  ...
  function calcularSaldoNuevo() {
    ...
    const totalPreview =
      document.getElementById("nuevo-total-preview");

    const adelantoPreview =
      document.getElementById("nuevo-adelanto-preview");

    const saldoPreview =
      document.getElementById("nuevo-saldo-preview");

    if (totalPreview)
      totalPreview.innerText = `Bs. ${formatearMonto(total)}`;

    if (adelantoPreview)
      adelantoPreview.innerText = `Bs. ${formatearMonto(adelanto)}`;

    if (saldoPreview)
      saldoPreview.innerText = `Bs. ${formatearMonto(saldo)}`;
  }
  ...
  formNuevo?.addEventListener("submit", async e => {
    ...
    const codigo =
      document.getElementById("nuevo-codigo")?.value.trim().toUpperCase()
      || generarCodigoAleatorio();

    const cliente =
      document.getElementById("nuevo-cliente")?.value.trim() || "";

    const mueble =
      document.getElementById("nuevo-mueble")?.value.trim() || "";

    const telefono =
      document.getElementById("nuevo-telefono")?.value.trim() || "";

    const presupuesto =
      Number(document.getElementById("nuevo-presupuesto")?.value) || 0;

    const adelanto =
      Number(document.getElementById("nuevo-adelanto")?.value) || 0;
  ...
  const filtroIngresos =
    document.getElementById("ingresos-mes");
  ...
  document.getElementById("portfolio-fotos")
    ?.addEventListener("change", mostrarPreviewArchivos);

  document.getElementById("portfolio-videos")
    ?.addEventListener("change", mostrarPreviewArchivos);

  document.getElementById("form-portafolio")
    ?.addEventListener("submit", publicarTrabajoPortafolio);
});
...
document.addEventListener("keydown", e => {
  const modal =
    document.getElementById("portfolio-gallery-modal");
