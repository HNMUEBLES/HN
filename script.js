// ============================================================
// HN MUEBLES - SCRIPT PRINCIPAL (Optimizado y Completo)
...
async function cargarIngresosDesdeNube() {
  ...
  try {
    ...
    ingresos = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
  } catch (error) { console.error("Error cargando ingresos:", error); }
}
...
async function buscarProyectoPublico(codigo) {
  ...
  try {
    const snapshot = await db.collection("proyectos_publicos").where("codigo", "==", codigo).limit(1).get();
    if (snapshot.empty) {
    ...
    const elCodigo = document.getElementById("res-codigo");
    const elMueble = document.getElementById("res-mueble");
    const elCliente = document.getElementById("res-cliente");
    const elEstado = document.getElementById("res-estado");
    const elPorcentaje = document.getElementById("res-porcentaje");
    const elDetalles = document.getElementById("res-detalles");
    const barraProgreso = document.getElementById("barra-progreso");

    if (elCodigo) elCodigo.innerText = data.codigo || "";
    if (elMueble) elMueble.innerText = data.mueble || "";
    if (elCliente) elCliente.innerText = `Cliente: ${data.cliente || ""}`;
    if (elEstado) elEstado.innerText = data.estado || "";
    if (elPorcentaje) elPorcentaje.innerText = `${data.progreso || 0}%`;
    if (elDetalles) elDetalles.innerText = data.detalles || "";
    if (barraProgreso) barraProgreso.style.width = `${data.progreso || 0}%`;
  } catch (error) {
...
function renderProyectosAdmin() {
  ...
  proyectos.forEach((p, index) => {
    ...
    const botones = etapas.map((estado, idx) => {
      ...
      return `<button type="button" onclick="cambiarEstadoPorId('${p.id}', ${idx}, ${(idx + 1) * 20})" style="border:none;padding:.4rem .7rem;border-radius:6px;cursor:pointer;font-size:.8rem;margin:.2rem;${activo ? "background:#f59e0b;color:#000;font-weight:bold;" : "background:rgba(255,255,255,.1);color:#fff;"}">${estado}</button>`;
    }).join("");
        ...
        <div style="display:flex;justify-content:space-between;gap:10px;flex-wrap:wrap;">
          <div style="flex:1;min-width:260px;">
            ...
            <div style="display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:10px;margin-top:12px;">
              <div class="financial-box" style="background:rgba(56,189,248,.07);border:1px solid rgba(56,189,248,.25);color:#38bdf8;"><span style="font-size:.75rem;">Monto total</span><strong style="font-size:1.05rem;white-space:nowrap;">Bs. ${formatearMonto(presupuesto)}</strong></div>
              <div class="financial-box" style="background:rgba(245,158,11,.07);border:1px solid rgba(245,158,11,.25);color:#f59e0b;"><span style="font-size:.75rem;">Adelanto</span><strong style="font-size:1.05rem;white-space:nowrap;">Bs. ${formatearMonto(adelanto)}</strong></div>
              <div class="financial-box" style="background:rgba(239,68,68,.07);border:1px solid rgba(239,68,68,.25);color:#ef4444;"><span style="font-size:.75rem;">Pendiente</span><strong style="font-size:1.05rem;white-space:nowrap;">Bs. ${formatearMonto(saldo)}</strong></div>
            </div>
...
// ==================== 8, 9, 10. ESTADOS, ELIMINAR Y EDITAR ====================
async function cambiarEstadoPorId(id, etapaIdx, progreso) {
  ...
  try {
    ...
    const publicoQuery = await db.collection("proyectos_publicos").where("codigo", "==", proyecto?.codigo).limit(1).get();
    publicoQuery.forEach(doc => batch.update(doc.ref, { estado, progreso, detalles }));
    await batch.commit();
    await cargarProyectosDesdeNube();
    renderProyectosAdmin();
  } catch (error) { console.error("Error estado:", error); }
}
...
async function eliminarProyecto(id) {
  ...
  try {
    ...
    if (proyecto) {
      const publicoSnap = await db.collection("proyectos_publicos").where("codigo", "==", proyecto.codigo).limit(1).get();
      publicoSnap.forEach(doc => batch.delete(doc.ref));
...
async function guardarEdicionInline(id, index) {
  ...
  try {
    ...
    if (!ingresoSnap.empty) {
      ...
      const cobrado = adelanto + (Number(ingresoDoc.data().pagosFinales) || 0);
...
function renderGestionIngresos() {
  ...
  if (filtro) {
    lista = lista.filter(ingreso => {
      if (!ingreso.fechaCreacion?.toDate) return true;
  ...
  if (document.getElementById("ing-resumen-proyectos")) document.getElementById("ing-resumen-proyectos").innerText = lista.length;
  if (document.getElementById("ing-resumen-cobrado")) document.getElementById("ing-resumen-cobrado").innerText = `Bs. ${formatearMonto(totalCobrado)}`;
  if (document.getElementById("ing-resumen-pendiente")) document.getElementById("ing-resumen-pendiente").innerText = `Bs. ${formatearMonto(totalPendiente)}`;
...
async function registrarPago(ingresoId) {
  if (!esAdmin || !auth.currentUser) return;
  const monto = Number(document.getElementById(`pago-${ingresoId}`)?.value) || 0;
  if (monto <= 0) return;
...
async function exportarIngresosPDF() {
  if (!esAdmin) return;
  const filtro = document.getElementById("ingresos-mes")?.value || "";
  if (!filtro || !window.jspdf?.jsPDF) return alert("jsPDF no disponible o sin filtro.");
  ...
  let totP = 0, totA = 0, totC = 0, totPen = 0;
  const filas = lista.map(i => {
    const t = Number(i.presupuesto) || 0;
    const a = Number(i.adelanto) || 0;
    const c = Number(i.cobrado) || 0;
    const pen = Math.max(t - c, 0);
    totP += t; totA += a; totC += c; totPen += pen;
    return [i.codigo || "", i.cliente || "", i.mueble || "", `Bs. ${formatearMonto(t)}`, `Bs. ${formatearMonto(a)}`, `Bs. ${formatearMonto(c)}`, `Bs. ${formatearMonto(pen)}`];
  });

  if (logo) doc.addImage(logo, "PNG", 14, 10, 25, 18);
  const posTxt = logo ? 45 : 14;

  doc.setTextColor(18, 18, 18);
  doc.setFont("helvetica", "bold"); doc.setFontSize(20);
  doc.text("HN MUEBLES", posTxt, 17);
  doc.setFont("helvetica", "normal"); doc.setFontSize(9); doc.setTextColor(105, 105, 105);
  doc.text("DISEÑO Y FABRICACIÓN DE MUEBLES A MEDIDA", posTxt, 23);
  doc.setFont("helvetica", "bold"); doc.setFontSize(14); doc.setTextColor(156, 113, 81);
  doc.text("REPORTE DE INGRESOS", 14, 38);
  doc.setFont("helvetica", "normal"); doc.setFontSize(11); doc.setTextColor(18, 18, 18);
  doc.text(`${nombresMes[Number(mes) - 1] || mes} ${anio}`, 14, 45);

  const gen = new Date();
  doc.setFontSize(8); doc.setTextColor(105, 105, 105);
  doc.text(`Generado el ${gen.toLocaleDateString("es-BO")} a las ${gen.toLocaleTimeString("es-BO", {hour: "2-digit", minute: "2-digit"})}`, 283, 45, { align: "right" });
  ...
  if (typeof doc.autoTable === "function") {
    doc.autoTable({
      ...
      body: filas,
      theme: "grid",
      headStyles: { fillColor: [18, 18, 18], textColor: [255, 255, 255], fontStyle: "bold", fontSize: 8, halign: "center" },
...
function notificarWhatsApp(index) {
  ...
  if (!numero.startsWith("591") && numero.length === 8) numero = "591" + numero;
...
async function cargarPortafolioAdmin() {
  ...
  } catch (error) { console.error("Error portafolio admin:", error); }
}
...
function renderPortafolioPublico() {
  ...
  portafolio.forEach((trabajo, idx) => {
    const media = Array.isArray(trabajo.media) ? trabajo.media : [];
    const pMedia = media[0];
    if (!pMedia) return;
    const card = document.createElement("article");
    card.className = "portfolio-card";
    card.innerHTML = `
      <div class="portfolio-media" onclick="abrirGaleriaPortafolio(${idx})" style="cursor:pointer;">
        ${pMedia.tipo === "video" ? `<video src="${pMedia.url}" muted playsinline preload="metadata"></video><div class="portfolio-video-badge"><i class="fa-solid fa-video"></i> Video</div>` : `<img src="${pMedia.url}" alt="" loading="lazy"/>`}
      ...
      <div class="portfolio-card-body">
        ...
        <button type="button" onclick="abrirGaleriaPortafolio(${idx})" style="margin-top:12px;width:100%;border:none;padding:10px;border-radius:7px;cursor:pointer;background:#9c7151;color:#fff;font-weight:bold;"><i class="fa-solid fa-expand"></i> Ver trabajo</button>
      </div>`;
    container.appendChild(card);
  });
}

let galeriaActual = 0, mediaActual = 0;
...
function abrirGaleriaPortafolio(idx) {
  if (!portafolio[idx]?.media?.length) return;
  crearModalGaleria();
  galeriaActual = idx;
  mediaActual = 0;
  actualizarGaleria();
  document.getElementById("portfolio-gallery-modal").style.display = "block";
  document.body.style.overflow = "hidden";
}

function actualizarGaleria() {
  const t = portafolio[galeriaActual];
  if (!t?.media?.length) return;
  if (mediaActual < 0) mediaActual = t.media.length - 1;
  if (mediaActual >= t.media.length) mediaActual = 0;
  const arch = t.media[mediaActual];
  ...
  content.innerHTML = arch.tipo === "video" ?
    `<video src="${arch.url}" controls autoplay playsinline style="max-width:90vw;max-height:82vh;border-radius:8px;"></video>` :
    `<img src="${arch.url}" alt="" style="max-width:90vw;max-height:82vh;object-fit:contain;border-radius:8px;"/>`;
  if (counter) counter.innerText = `${mediaActual + 1} / ${t.media.length}`;
}

function mediaGaleriaAnterior() { mediaActual--; actualizarGaleria(); }
function mediaGaleriaSiguiente() { mediaActual++; actualizarGaleria(); }
function cerrarGaleriaPortafolio() {
  const m = document.getElementById("portfolio-gallery-modal");
...
function mostrarPreviewArchivos() {
  ...
  [...fotos, ...videos].forEach(archivo => {
    ...
    div.innerHTML = archivo.type.startsWith("video/") ? `<video src="${url}" muted></video><span class="file-preview-type"><i class="fa-solid fa-video"></i> Video</span>` : `<img src="${url}" alt=""><span class="file-preview-type"><i class="fa-solid fa-image"></i> Foto</span>`;
...
async function publicarTrabajoPortafolio(e) {
  e.preventDefault();
  if (!esAdmin || !auth.currentUser) return alert("Inicia sesión como administrador.");
  ...
  if (!titulo || !archivos.length) return alert("Escribe un título y selecciona al menos una foto o video.");
  ...
  try {
    ...
    for (let i = 0; i < archivos.length; i++) {
      ...
      if (progText) progText.innerText = `Subiendo ${i + 1} de ${archivos.length}: ${arch.name}`;
      const res = await subirArchivoCloudinary(arch, pTot => {
        if (progBar) progBar.style.width = `${((i + pTot / 100) / archivos.length) * 100}%`;
      });
      media.push(res);
    }

    if (progText) progText.innerText = "Guardando información...";
    await db.collection("portafolio").add({ titulo, descripcion, media, creadoPor: auth.currentUser.email, creadoEn: firebase.firestore.FieldValue.serverTimestamp() });
    alert("Trabajo publicado correctamente.");
    document.getElementById("form-portafolio")?.reset();
    if (document.getElementById("portfolio-files-preview")) document.getElementById("portfolio-files-preview").innerHTML = "";
...
function renderPortafolioAdmin() {
  ...
  portafolio.forEach((trabajo, index) => {
    ...
    const pMedia = trabajo.media?.[0];
    card.innerHTML = `
      ${pMedia?.tipo === "video" ? `<video src="${pMedia.url}" class="portfolio-admin-thumb" muted preload="metadata"></video>` : `<img src="${pMedia?.url || ""}" class="portfolio-admin-thumb" alt="">`}
      <div class="portfolio-admin-info"><strong>${escaparHTML(trabajo.titulo || "")}</strong><p>${escaparHTML(trabajo.descripcion || "")}</p><p>${trabajo.media?.length || 0} archivo(s)</p></div>
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (formLogin) {
    formLogin.addEventListener("submit", async e => {
      ...
      try {
        ...
        if (document.getElementById("input-pass")) document.getElementById("input-pass").value = "";
      } catch (error) {
  ...
  // Auto-buscar si viene un código por parámetro en la URL (ej: ?codigo=HN123)
  ...
  function calcularSaldoNuevo() {
    ...
    if (document.getElementById("nuevo-total-preview")) document.getElementById("nuevo-total-preview").innerText = `Bs. ${formatearMonto(total)}`;
    if (document.getElementById("nuevo-adelanto-preview")) document.getElementById("nuevo-adelanto-preview").innerText = `Bs. ${formatearMonto(adelanto)}`;
    if (document.getElementById("nuevo-saldo-preview")) document.getElementById("nuevo-saldo-preview").innerText = `Bs. ${formatearMonto(saldo)}`;
  }
  ...
  if (formNuevo) {
    formNuevo.addEventListener("submit", async e => {
      ...
      } catch (error) { console.error("Error creando:", error); alert("No se pudo guardar el proyecto."); }
    });
  }
